...
枚举使用规范:
    ...
    2. BaseSchema 已配置 use_enum_values=True：
       ...
       - 出参序列化：API JSON / model_dump(mode="json") 输出 .value（int/str）
       - 普通 model_dump() 对未经校验的默认枚举值不保证转换；需要时为字段启用 validate_default
...
__all__ = ["UserCreate", "UserResponse", "UserSearch", "UserSearchResponse"]
