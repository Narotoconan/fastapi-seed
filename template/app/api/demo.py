...
async def get_user_list_api(
    ...
    total = 56
    datas = [
        {"name": fake.name(), "email": fake.email(), "date": fake.date_time()} for _ in range(pagination.page_size)
    ]
...
async def export_user_api() -> StreamingResponse:
    """
...
async def error_demo_api(
    error_type: str = Query(default="biz", description="biz / auth / internal"),
) -> ResponseSchema:
    """手动触发不同类型异常 — 仅供开发调试"""
    if error_type == "biz":
...
async def search_user(params: Annotated[UserSearch, Query()]) -> ResponseSchema:
